    ...
    <c:set var="url" value="${pageContext.request.contextPath}" />
            ...
            <c:forEach var="order" items="${orders}">
                <c:if test="${order.id == param.orderId}">
                    ...
                    <fmt:parseDate value="${order.createdAt.toString()}" pattern="yyyy-MM-dd'T'HH:mm" var="parsedDate"
                        type="both" />
                    ...
                    <c:set var="orderJson">
                        {id:${order.id},code:'${order.orderCode}',status:'${order.status}',totalPrice:'
                        <fmt:formatNumber value="${order.totalPrice}" type="currency" currencySymbol="₫" />
                        ',paymentMethod:'${order.paymentMethod}',customerName:'${order.account.profile.fullName.replace("'",
                        "\\'")}',customerPhone:'${order.account.profile.phoneNumber}',customerEmail:'${order.account.email}',shippingAddress:'${order.shippingAddress.replace("'",
                        "\\'")}',customerAvatar:'${not empty order.account.profile.avatarUrl ?
                        url.concat(order.account.profile.avatarUrl) :
                        "https://api.dicebear.com/7.x/avataaars/svg?seed=".concat(order.account.id)}',time:'${formattedDate}',items:[
                        <c:forEach var="item" items="${order.orderDetails}" varStatus="i">
                            {id:${item.id},name:'${item.product.name.replace("'", "\\'")}',image:'${url}${not empty
                            item.productVariant.imageUrl ? item.productVariant.imageUrl :
                            item.product.images[0].url}',variant:'${item.productVariant.sku}',quantity:${item.quantity},price:'
                                ...
                                <div class="relative" x-data="{ openMenu: false }">
                                    ...
                                    <div x-show="openMenu" x-cloak
                                        ...
                                        <div class="py-2 text-sm">
                                            <a href="${url}/order-details/${order.orderCode}"
