...
<c:forEach var="order" items="${orders}">
    <c:if test="${order.id == param.orderId}">
        ...
        <!-- Prepare JSON Data on a single line to avoid JS errors -->
        ...
        <div class="order-card group glass-card p-5 rounded-[24px] hover:shadow-2xl hover:bg-white/80 transition-all duration-300 cursor-pointer active:scale-[0.98]" 
             data-status="${order.status}"
             @click="openDetail(${orderJson})">
            <div class="flex items-center gap-6">
                ...
                <div class="w-32 flex-shrink-0">
                    ...
                    <div class="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-wider">
                        ${formattedDate.substring(0, 5)} - ${formattedDate.substring(8, 13)}
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 w-48 flex-shrink-0">
                    ...
                    <div class="min-w-0">
                        <div class="font-bold text-sm text-small-navy truncate">${order.account.profile.fullName}</div>
                        <div class="text-[10px] text-gray-400 font-medium uppercase tracking-tight">Customer</div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 w-40 flex-shrink-0">
                    ...
                    <c:if test="${order.orderDetails.size() > 2}">
                        <div class="text-[10px] text-gray-400 font-bold tracking-tight">+${order.orderDetails.size() - 2} items</div>
                    </c:if>
                </div>
                ...
                <div class="flex items-center gap-3 w-44 flex-shrink-0">
                    ...
                    <div class="p-1.5 bg-gray-50 rounded-lg text-gray-400">
                        <c:choose>
                            <c:when test="${order.paymentMethod == 'COD'}"><i class="fas fa-box text-xs"></i></c:when>
                            <c:when test="${order.paymentMethod == 'QR'}"><i class="fas fa-qrcode text-xs"></i></c:when>
                ...
                <div class="flex-1 flex items-center gap-2">
                    ...
                    <c:choose>
                        <c:when test="${order.status == 'PENDING'}"><c:set var="statusClass" value="bg-amber-50 text-amber-600 border-amber-100/50" /></c:when>
                        <c:when test="${order.status == 'CONFIRMED'}"><c:set var="statusClass" value="bg-blue-50 text-blue-600 border-blue-100/50" /></c:when>
                        <c:when test="${order.status == 'SHIPPING'}"><c:set var="statusClass" value="bg-indigo-50 text-indigo-600 border-indigo-100/50" /></c:when>
                        <c:when test="${order.status == 'DELIVERED'}"><c:set var="statusClass" value="bg-emerald-50 text-emerald-600 border-emerald-100/50" /></c:when>
                        <c:otherwise><c:set var="statusClass" value="bg-red-50 text-red-600 border-red-100/50" /></c:otherwise>
                    </c:choose>
                    <span class="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.1em] border ${statusClass}">
                ...
                <div class="flex items-center gap-3 flex-shrink-0">
                    <c:choose>
                        <c:when test="${order.status == 'PENDING'}">
                            <button onclick="event.stopPropagation(); updateStatus(${order.id}, 'CONFIRMED')" 
                        ...
                        <c:when test="${order.status == 'SHIPPING'}">
                            <button onclick="event.stopPropagation(); updateStatus(${order.id}, 'DELIVERED')" 
                    ...
                    <div class="relative" x-data="{ openMenu: false }">
                        <button @click="event.stopPropagation(); openMenu = !openMenu" @click.away="openMenu = false" 
                        ...
                        <div x-show="openMenu" x-cloak
                            ...
                            <div class="py-2 text-sm">
                                <a href="${url}/order-details/${order.orderCode}" class="flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-all">
                                    <i class="fas fa-eye w-5"></i> Chi tiết trang mới
                                </a>
                                <c:if test="${order.status == 'PENDING'}">
                                    <button onclick="event.stopPropagation(); updateStatus(${order.id}, 'CANCELLED')" class="w-full flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 transition-all">
