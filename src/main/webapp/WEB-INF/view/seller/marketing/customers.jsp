...
<head>
    ...
    <style>
        ...
        .robot-fab {
            ...
            background: #10b981; color: white; border-radius: 18px;
...
<body class="text-slate-600">
            ...
            <header class="flex items-center justify-between mb-10">
                ...
                <div class="relative">
                    <input type="text" placeholder="Tìm kiếm khách hàng..." 
                           class="pl-12 pr-6 py-2.5 bg-white border border-slate-100 rounded-2xl text-sm w-72 focus:outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500 transition-all shadow-sm">
                        ...
                        <div class="rank-card silver-card w-64 rounded-[32px] p-1 shadow-xl shadow-slate-400/20">
                            <div class="bg-slate-700/10 rounded-[31px] p-6 text-center text-white">
                                <div class="relative inline-block mb-4">
                                    <c:choose>
                                        <c:when test="${not empty c2.avatarUrl}">
                                            <img src="${c2.avatarUrl}" class="w-20 h-20 rounded-full border-4 border-white/30 object-cover shadow-lg">
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <div class="bg-white/10 rounded-2xl p-4">
                                    ...
                                    <span class="text-xl font-bold italic">
                                        <fmt:formatNumber value="${c2.totalSpent}" type="currency" currencySymbol="₫" maxFractionDigits="0" />
                                    </span>
                                </div>
                                <div class="flex justify-center gap-3 mt-4">
                                    ...
                                    <div class="bg-white/5 rounded-xl px-3 py-2">
                                        ...
                                        <span class="font-bold text-sm">${fn:substringBefore(c2.totalSpent/10000, '.')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="rank-card gold-card w-72 rounded-[40px] p-1 shadow-2xl shadow-yellow-500/30 -translate-y-4">
                            <div class="bg-white/5 rounded-[39px] p-8 text-center text-white">
                                <div class="relative inline-block mb-6">
                                    <c:choose>
                                        <c:when test="${not empty c1.avatarUrl}">
                                            <img src="${c1.avatarUrl}" class="w-24 h-24 rounded-full border-4 border-white/50 object-cover shadow-xl">
                                        </c:when>
                                        <c:otherwise>
                                            <div class="w-24 h-24 rounded-full bg-white/30 flex items-center justify-center text-3xl font-black border-4 border-white/50 shadow-inner">
                                                ${fn:substring(c1.fullName, 0, 1)}
                                            </div>
                                        </c:otherwise>
                                    </c:choose>
                                ...
                                <div class="bg-white/20 rounded-[28px] p-6 backdrop-blur-sm border border-white/10">
                                    ...
                                    <span class="text-2xl font-black italic">
                                        <fmt:formatNumber value="${c1.totalSpent}" type="currency" currencySymbol="₫" maxFractionDigits="0" />
                                    </span>
                                </div>
                                <div class="flex justify-center gap-4 mt-6">
                                    ...
                                    <div class="bg-white/10 rounded-2xl px-4 py-3">
                                        ...
                                        <span class="font-bold text-lg">${fn:substringBefore(c1.totalSpent/10000, '.')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <c:if test="${fn:length(customers) >= 3}">
                        ...
                        <div class="rank-card bronze-card w-64 rounded-[32px] p-1 shadow-xl shadow-orange-500/20">
                            <div class="bg-orange-700/10 rounded-[31px] p-6 text-center text-white">
                                <div class="relative inline-block mb-4">
                                    <c:choose>
                                        <c:when test="${not empty c3.avatarUrl}">
                                            <img src="${c3.avatarUrl}" class="w-20 h-20 rounded-full border-4 border-white/30 object-cover shadow-lg">
                                        </c:when>
                                        <c:otherwise>
                                            <div class="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center text-2xl font-bold border-4 border-white/30">
                                                ${fn:substring(c3.fullName, 0, 1)}
                                            </div>
                                        </c:otherwise>
                                    </c:choose>
                                ...
                                <div class="bg-white/10 rounded-2xl p-4">
                                    <span class="block text-[10px] text-white/60 uppercase font-bold mb-1">Tổng chi tiêu</span>
                                    <span class="text-xl font-bold italic">
                                        <fmt:formatNumber value="${c3.totalSpent}" type="currency" currencySymbol="₫" maxFractionDigits="0" />
                                    </span>
                                </div>
                                <div class="flex justify-center gap-3 mt-4">
                                    <div class="bg-white/5 rounded-xl px-3 py-2">
                                    ...
                                    <div class="bg-white/5 rounded-xl px-3 py-2">
                                        <span class="block text-[9px] opacity-60">Điểm</span>
                                        <span class="font-bold text-sm">${fn:substringBefore(c3.totalSpent/10000, '.')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex justify-between items-center mb-8 px-4">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <div class="bg-slate-50 rounded-xl px-3 py-2">
                                            ...
                                            <span class="font-bold text-xs">${fn:substringBefore(customer.totalSpent/10000, '.')}</span>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function appendMsg(text, isUser) {
            ...
            // self-start = trái (AI), self-end = phải (User)
            msgDiv.className = `${isUser ? 'self-end' : 'self-start'} max-w-[85%]`;
            const bubbleDiv = document.createElement('div');
            bubbleDiv.className = `px-5 py-4 rounded-[24px] shadow-sm text-sm whitespace-pre-line leading-relaxed ${isUser ? 'bubble-user' : 'bubble-ai'}`;
