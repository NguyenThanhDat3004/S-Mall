...
<head>
    ...
    <title>${otpSent ? 'Xác thực OTP' : 'Đăng ký tài khoản'} | S-Mall</title>
...
<body class="auth-body">
    ...
    <c:set var="cardWidth" value="${otpSent ? '450px' : '600px'}" />
    <div class="auth-card" style="max-width: ${cardWidth};">
        <a href="${pageContext.request.contextPath}/" class="auth-logo">S-Mall</a>
        
        <c:choose>
            <c:when test="${otpSent}">
                <!-- STEP 2: OTP Verification Fragment -->
                <%@ include file="_otp-fragment.jsp" %>
            </c:when>
            <c:otherwise>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 1. Recover all data with Error Protection
            try {
